...
{
	m_tMFSock = 0;
	TRACE("MFSockInterface::MFSockInterface()\n");
}
...
BOOL MFSockInterface::Initialize(void* pvParams)
{
	...
	TRACE("MFSockInterface::Initialize()\n");
	...
	// ----------------------------------------------------------------------------
	// The IDGenerator has to be initialized only once.
	// ----------------------------------------------------------------------------
	if (m_pPIDGen == NULL) {
	...
	TRACE("MFSockInterface::Initialize - Using PID %d\n", m_nPID);
	return TRUE;
}
...
BOOL MFSockInterface::ConnectToServer()
{
	...
	TRACE("MFSockInterface::ConnectToServer()\n");
...
void MFSockInterface::Disconnect()
{
	...
	TRACE("MFSockInterface::Disconnect()\n");
